...
import { useTheme } from "react-router-themes";
import {
  // Core system
  createEditorSystem,
...
function EditorContent({
  ...
  useEffect(() => {
    ...
    return editor.registerUpdateListener((listener) => {
      setTimeout(() => {
        const editorState = listener.editorState;
        const prevEditorState = listener.prevEditorState;

        let isFoundImage = false;

        Array.from(prevEditorState._nodeMap.entries())
            .filter(prevNode => prevNode[1].__type === 'image').map(prevNode => {
          isFoundImage = Array.from(editorState._nodeMap.entries())
              .filter(node => node[1].__type === 'image')
              .some(node => node[1].__src === prevNode[1].__src);

          if (!isFoundImage) {
            if (prevNode[1].__src.includes('blob')) {
              setDeleteFileHandler ? setDeleteFileHandler(prevNode[1].__alt) : null;
            }
          }
        });
...
export const DefaultTemplate = forwardRef<DefaultTemplateRef, DefaultTemplateProps>(({ className, onReady, onChange, setNewFileHandler, setDeleteFileHandler }, ref) => {
  ...
  useEffect(() => {
    imageExtension.configure({
      uploadHandler: async (file: File) => {
        setNewFileHandler ? setNewFileHandler(file) : null;

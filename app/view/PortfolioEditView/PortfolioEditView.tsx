...
const PortfolioEditView = (
    ...
    const submitHandler = (data: IWork) => {
        ...
        const response = rootStore.updateWork(updateWork, files, editorNewFiles)
            .then(response => {
                if (response) {
                    setIsSuccess(true);
                    setCurrWork(response);
                    setFiles([]);
                    setNewPhotos([]);
                    setPhotos(response.images);
                    setEditorText(response.description);
                    setEditorNewFiles([]);
                }
            });
    }
    ...
    return (
        <>
            ...
            <SuccessMessage message={'Работа успешно сохранена!'} activate={isSuccess} setActivate={setIsSuccess} />
        </>
    );
};
